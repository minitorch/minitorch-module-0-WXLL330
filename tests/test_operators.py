...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert 0.0 <= sigmoid(a) <= 1
    assert pytest.approx(1 - sigmoid(a)) == sigmoid(-a)
...
def test_distribute(x: float, y: float, z: float) -> None:
    ...
    """
    # TODO: Implement for Task 0.2.
    assert pytest.approx(mul(z, add(x, y))) == add(mul(z, x), mul(z, y))
    # raise NotImplementedError("Need to implement for Task 0.2")
...
def test_other(x: float, y: float) -> None:
    ...
    # TODO: Implement for Task 0.2.
    if x != 0:
        assert pytest.approx(x) == inv(inv(x))

        expected_inv_back = y * (-1.0 / ((x + 1) ** 2))
        assert inv_back(x + 1, y) == pytest.approx(expected_inv_back)
    ...
    if x > 0:
        ...
        assert relu_back(x, y) == pytest.approx(0.0)
    # raise NotImplementedError("Need to implement for Task 0.2")
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    assert_close(sum(ls1) + sum(ls2), sum1 + sum2)
